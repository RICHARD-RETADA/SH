  ...
  api.markAsReadAll((err) => {
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}request`) {
        ...
        return api.sendMessage(`» A request has been sent to the bot admin(s)!`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`» Your box has not been approved\n» To submit an approval request, use:\n\n» ${prefix}request`, event.threadID);
    };
		switch (event.type) {
			case "message":
			case "message_reply":
