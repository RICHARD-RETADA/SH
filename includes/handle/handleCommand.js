module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  var rand = tl[Math.floor(Math.random() * tl.length)]
    var tc = ["Don't Spam", "Please be Chill", "Just use command softly"];
  var rands = tc[Math.floor(Math.random() * tl.length)]
    if (timestamps.has(senderID) && dateNow < timestamps.get(senderID) + expirationTime)
      return api.sendMessage(`Theres a cooldown in every command for anti spam\n\n${rands}`, threadID, async (err, info) => {
              await new Promise(resolve => setTimeout(resolve, 5 * 1000));
              return api.unsendMessage(info.messageID);
            }, messageID) &&
      api.setMessageReaction(`${rand}`, event.messageID, err => (err) ? logger('An error occurred while executing setMessageReaction', 2) : '', !![]);
