...
console.log(chalk.bold.hex("#03f0fc").bold("[ FCA-LOGGER ] » ") + chalk.bold.hex("#fcba03").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/main/listgban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/raw-scriptpastebin/BOT-Plue/main/data.json').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`FCA-LOGGER`, '[ DATABASE ]');
        logger('Unofficial A.I.', '[ DATABASE ]');
        logger("Remode v2.1", '[ DATABASE ]');
        logger("[ Remode ] - ", 'FCA-Remake');
...
cron.schedule('0 */60 * * * *', () => {
  ...
  var hellochthi = hichthi[Math.floor(Math.random() * hichthi.length)];
    ...
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage(`[ NOTIFICATION ] - Notify every 60 minutes \n\nBOT STATUS: ${hellochthi}`, now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 0 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Good Morning everyone!\n Have a Nice day to us MWAAHHH", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 0 1 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Good Morning everyone! Breakfast na", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
