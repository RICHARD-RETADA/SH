...
module.exports.handleEvent = async ({ api, event, args, Threads }) => {
    ...
    if (sim.hasOwnProperty(threadID) && sim[threadID] == true) {
      ...
      axios.get(encodeURI(`https://api.phamvandien.xyz/sim?type=ask&ask=${event.body}`)).then(res => {
            if (res.data.answer == "null" || res.data.answer == "I don't understand anything!!!.") {
                api.sendMessage("Please Teach me\n !sim3 teach hi => hello",threadID,messageID)
            } else {
...
module.exports.run = async ({ api, event, args, Threads }) => {
    ...
    if (!args[0]) { api.sendMessage("I don't know what your'e saying please teach me", threadID, messageID) } else {
        switch(args[0]) {
            ...
            default:
            axios.get(encodeURI(`https://api.phamvandien.xyz/sim?type=ask&ask=${args}`)).then(res => {
            if (res.data.answer == "null" || res.data.answer == "I don't understand anything!!!.") {
