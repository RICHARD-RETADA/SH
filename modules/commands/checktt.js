...
module.exports.run = async function ({ api, event, args, Users, Threads }) {
    ...
    if (!fs.existsSync(path + threadID + '.json')) {
        return api.sendMessage("No data yet", threadID);
    }
    const threadData = JSON.parse(fs.readFileSync(path + threadID + '.json'));
    ...
    if ((!check && Object.keys(mentions).length == 0) || (!check && Object.keys(mentions).length == 1) || (!check && event.type == 'message_reply')) {
        ...
        if (userRank == -1) {
            return api.sendMessage(`${target} no data yet`, threadID);
        }
        body += `
        👥 Name: ${nameUID}
        💬 Message of the Week: ${userTotalWeek}
        💬 Message of the Day: ${userTotalDay}
        📝 Total: ${userTotal} (Top ${userRank + 1})
        `.replace(/^ +/gm, '');
    } else {
        body = storage.map(item => {
            return `${count++}. ${item.name} (${item.count})`;
        }).join('\n');
        footer = `↠ Total Message: ${storage.reduce((a, b) => a + b.count, 0)}`;
    }
