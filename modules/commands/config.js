...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  if (type == 'menu') {
    if (["01", "1", "02", "2"].includes(args[0])) {
      reply(`Please reply to this message with ${["01", "1"].includes(args[0]) ? "bio" : "bio"} you want to exchange for bot or 'delete' if you want to delete ${["01", "1"].includes(args[0]) ? "bio" : "bio"} present`, (err, info) => {
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (err, data) => {
        ...
        else reply(`» Already ${args[1] == 'on' ? 'Turn on' : 'Turn off'} Successful bot avatar shield`);
      });
    }
    else if (["08", "8"].includes(args[0])) {
      return reply(`Reply to this message with the id of the person you want to block, you can enter multiple ids separated by spaces or newlines`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      return reply(`Reply to this message with the post id you want to delete, you can enter multiple ids separated by spaces or carriage returns`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
      ...
      reply(`Reply to this message with the post id you want ${args[0]  == "13" ? "release emotions" : args[0] == "14" ? "send friend invitations" : args[0] == "15" ? "accept friend request" : args[0] == "16" ? "reject friend request" : args[0] == "17" ? "delete friends" : "send Message"}, can enter multiple ids separated by space or newline`, (e, info) => {
        global.client.handleReply.push({
          name: this.config.name,
          messageID: info.messageID,
          author: senderID,
    ...
    api.changeBio(bio, false, (err) => {
      ...
      else return reply(`Already ${!bio ? "delete bot's profile successfully" : `change bot profile to: ${bio}`}`);
    });
  }
    ...
    api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
      if (e) return reply(`An error occurred, please try again later`);
      else if (JSON.parse(i).errors) reply(`An error occurred: ${JSON.parse(i).errors[0].summary}, ${JSON.parse(i).errors[0].description}`);
      else reply(`Already ${!nickname ? "delete bot's alias name successfully" : `rename bot's nickname to: ${nickname}`}`);
    });
  }
    ...
    else return reply(`Please enter a valid image link or reply to a message with an image you want to set as an avatar for the bot`, (err, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    try {
      ...
      api.httpPost("https://www.facebook.com/api/graphql/", form, (e, i) => {
        ...
        else if (JSON.parse(i.slice(0, i.indexOf('\n') + 1)).errors) reply(`An error occurred: ${JSON.parse(i).errors[0].description}`);
        else reply(`Changed the bot avatar successfully`);
      });
    }
    catch(err) {
    ...
    if (!body) return reply("Please enter the uid of the people you want to block on messenger, you can enter multiple id's separated by space or newline", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    if (!body) return reply("Please enter uid of the people you want to unblock on messenger, can enter multiple ids separated by space or newline", (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`» Successfully unblocked ${success.length} users on messenger${failed.length > 0 ? `\n» Unblock failure ${failed.length} user, id: ${failed.join(" ")}` : ""}`);
  }
    ...
    reply(`» Successfully commented ${success.length} Article ${failed.length > 0 ? `\n» Comment failed ${failed.length} post, postID: ${failed.join(" ")}` : ""}`);
  }
    ...
    for (const postID of postIDs) {
  		...
  		try {
           ...
           headers
  		  ...
  		  reply("An error occurred, the article id does not exist or you are not the owner of this article");
  		}
    ...
    reply(`» Successfully deleted ${success.length} Article ${failed.length > 0 ? `\n» Delete failed ${failed.length} post, postID: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'choiceIdReactionPost') {
    if (!body) return reply(`Please enter the id of the post you want to react`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`Enter the emotion you want to react to for ${listID.length} posts (unlike/like/love/heart/haha/wow/sad/angry)`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`» Released emotion ${feeling} for ${success.length} successful article ${failed.length > 0 ? `» Reaction failed ${failed.length} post, postID: ${failed.join(" ")}` : ''}`);
  }
    ...
    reply(`» Friend request sent to ${success.length} id${failed.length > 0 ? `\n» Send a friend request to ${failed.length} failed id: ${failed.join(" ")}` : ""}`);
  }
  
  
  else if (type == 'choiceIdSendMessage') {
    const listID = body.replace(/\s+/g, " ").split(" ");
    reply(`Enter the text of the message you want to send to ${listID.length} user`, (e, info) => {
      global.client.handleReply.push({
        name: this.config.name,
        messageID: info.messageID,
        author: senderID,
    ...
    reply(`» Successfully deleted ${success.length} friends${failed.length > 0 ? `\n» Delete failed ${failed.length} friends:\n${failed.join("\n")}` : ""}`);
  }
    ...
    reply(`» Đã ${type == 'acceptFriendRequest' ? 'accept' : 'erase'} successful friend request of ${success.length} id${failed.length > 0 ? `\n» Failed with ${failed.length} id: ${failed.join(" ")}` : ""}`);
  }
    ...
    .then(response => {
      ...
      reply(`Successful note creation, link: ${'https://buildtool.dev/code-viewer.php?' + href}`)
    })
    .catch(err => {
      reply('An error occurred, please try again later');
    })
