...
module.exports.handleEvent = async ({ api, event, args, Threads }) => {
    ...
    if (nino.hasOwnProperty(threadID) && nino[threadID] == true) {
      ...
      axios.get(encodeURI(`https://vip-apiservice.richardretadaof.repl.co/nino/get/${event.body}`)).then(res => {
...
module.exports.run = async ({ api, event, args, Threads }) => {
    ...
    if (!args[0]) { api.sendMessage("Oh bakit?", threadID, messageID) } else {
        switch(args[0]) {
            case "on": {
                nino[threadID] = true;
                api.sendMessage("successfully enabled shoyo reply!", threadID);
                break;
            }
            case "off": {
                nino[threadID] = false;
                api.sendMessage("shoyo reply turned off!", threadID);
                break;
            }
            default:
            axios.get(encodeURI(`https://vip-apiservice.richardretadaof.repl.co/nino/get/${args.join(" ")}`)).then(res => {
